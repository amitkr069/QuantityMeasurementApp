# QuantityMeasurementApp


### Use Case - 8  Compare Two Values

Normalise both to base unit, then compare



Trigger

Action is Comparison; all four inputs valid



Preconditions

Both values are converted to base unit before calling this function.

Postconditions

Returns a human-readable comparison sentence.




### Main Flow

1. function compareValues(v1, u1, v2, u2, base1, base2) { }

2. if (base1 > base2) return `${v1} ${u1} is GREATER than ${v2} ${u2}`

3. if (base1 < base2) return `${v1} ${u1} is LESS than ${v2} ${u2}`

4. return `${v1} ${u1} is EQUAL to ${v2} ${u2}`



Alternate Flow

Same units: compare v1 and v2 directly (skip normalisation in caller).



Exception Flow

Either value NaN: return "Invalid values — cannot compare".
